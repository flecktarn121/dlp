...
public class TypeCheckingVisitor extends AbstractVisitor<Type, Void> {
	...
	if (e.getParameters().size() != ((FunctionType) e.getName().getDefinition().getType()).getParamType().size()) {
		...
		Type requiredParamType = ((FunctionType) e.getName().getDefinition().getType()).getParamType().get(i)
			.getType();
	...
	if (e.getValue().getType().isComplex()) {
	    ...
	    ErrorHandler.getInstance().addError(error);
	} else if (!e.getValue().getType().getName().equals(param.getName())) {
